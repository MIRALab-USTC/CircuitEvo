...
def refresh(temp_program_path, output_path):
    ...
    command = f"./abc -c 'read_eqn {temp_program_path}; strash; write_aiger {temp_aig_path}; \
        read_aiger {temp_aig_path}; write_eqn {temp_program_path}'"
...
def fix_program(program_path, indices_mapping, signal): 
    ...
    PO_list = [item.replace(';', ' ').split() for item in lines if item.strip().startswith('OUTORDER')][0][2:]
    ...
    while len(PO_list) < len(indices_mapping):
        max_number += 1
        new_PO = PO_list[0].replace('0', str(max_number))
    ...
    for i in range(len(new_lines)):
        if new_lines[i].strip().startswith('OUTORDER'):
            ...
            new_lines[i] = f"OUTORDER = {outorder_row};\n"
    with open(program_path, 'w') as f:
